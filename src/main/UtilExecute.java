...
public class UtilExecute {
	...
	/**
	 * Genere un code promo Aleatoire Normalement on doit donner au client un code
	 * promo , si il depasse 100euro d'achat
	 * 
	...
	public String generateCodePromo() {
		...
		for (int x = 0; x < 10; x++) { // j'ai pris par defaut la longeur du code promo = 10
			int i = (int) Math.floor(Math.random() * chars.length() - 1); // Si tu supprimes des lettres tu diminues ce
																			// nb
			pass += chars.charAt(i);
		}
	...
	public int commandeChoix() {
		...
		int choix;
		System.out.println("* ETAPE 1 : choisir un type d'impression");
	...
	public float showCodePromo(String email, float prix) {

		ArrayList<CodeUtilisateur> codesPromo = new ArrayList<>();
		Consulter consulter = new Consulter();
		codesPromo = consulter.getClientCodeUser(email);
		float prixAfter;

		if (codesPromo.size() > 0) {
			System.out.println("Suite a votre derniere Achat vous avez une reduction de "
					+ codesPromo.get(0).getPourcentage() + "% sur votre commande");
			prixAfter = (float) (prix - (prix * (codesPromo.get(0).getPourcentage() / 100)));
